...
Func _viExecCommand($h_session, $s_command, $i_timeout_ms = -1, $s_mode = @LF)
	If StringInStr($s_command, "?") = 0 Then
		...
		__viPrintf($h_session, $s_command, $i_timeout_ms, $s_mode)
		Return
	Else
...
Func _viOpen($s_visa_address)
	...
	Local $a_results
	$a_results = DllCall($hVISAdll, "int", "viOpen", "long", $VISA_DEFAULT_RM, "str", $s_visa_address, "long", 0, "long", 0, "long*",$h_session)
	If @error Then Return SetError(@error, @extended, -1)
	Local $errStatus = $a_results[0]
	If $errStatus <> 0 Then
		; Could not open VISA instrument/resource
		MsgBox(16,"VISA error","Error: " &Hex($errStatus,8)& ",could not open VISA instrument/resource: " & $s_visa_address)
...
Func _viClose($h_session)
	...
	; viClose(h_session);
	Local $a_results
	$a_results = DllCall($hVISAdll, "int", "viClose", "int", $h_session)
	If @error Then Return SetError(@error, @extended, -1)
	Local $errStatus = $a_results[0]
	If $errStatus <> 0 Then
...
Func _viFindGpib(ByRef $a_descriptor_list, ByRef $a_idn_list, $f_show_search_results = 0)
	...
	Local $a_results = DllCall($hVISAdll, "long", "viFindRsrc", _
	...
	For $n = 1 To $i_num_instr - 1
		...
		$a_results = DllCall($hVISAdll, "long", "viFindNext", "long", $h_list_pointer, "str", "")
		If @error Then Return SetError(@error, @extended, -1)
...
Func __viOpenDefaultRM()
	...
	If $VISA_DEFAULT_RM < 0 Then
		...
		$hVISAdll = DllOpen("visa32")
		If @error Then
		    MsgBox(16,"VISA error","Could not open VISA32 dll. Check if DLL is installed in Windows/system32 folder.")
		    Return SetError(1,0,-1)
		EndIf
		; errStatus = viOpenDefaultRM (&VISA_DEFAULT_RM);
		; signed int viOpenDefaultRM(*unsigned long)
		Local $a_results
		$a_results = DllCall($hVISAdll, "int", "viOpenDefaultRM", "long*", $VISA_DEFAULT_RM)
		If @error Then Return SetError(@error, @extended, -1)
		Local $errStatus = $a_results[0]
		If $errStatus <> 0 Then
		...
		ConsoleWrite(" viOpenDefaultRM returned " &Ubound($a_results) & " parameters " & @CRLF)
		$VISA_DEFAULT_RM = $a_results[1]
		ConsoleWrite("VISA session: " &$VISA_DEFAULT_RM & @CRLF)
		If $VISA_DEFAULT_RM <= 0 Then
			; There was an error, reset the $VISA_DEFAULT_RM
			MsgBox(16,"VISA error","Session error: " & Hex($VISA_DEFAULT_RM,8))
...
Func __viPrintf($h_session, $s_command, $i_timeout_ms = -1, $s_option = @LF)
	...
	Select
		Case $s_option = "str"
			...
			$a_results = DllCall($hVISAdll, "int:cdecl", "viPrintf", "int", $h_session, "str", "%s", "str", $s_command) ; Call viPrintf with escape sequence protection
		Case ($s_option = @CR Or $s_option = @LF Or $s_option = @CRLF)
			; Append the selected terminator to the SCPI command
			$a_results = DllCall($hVISAdll, "int:cdecl", "viPrintf", "int", $h_session, "str", $s_command & $s_option)
		Case Else ; In all other cases, ignore the "mode" and do not use any terminator string
			$a_results = DllCall($hVISAdll, "int:cdecl", "viPrintf", "int", $h_session, "str", $s_command) ; Call viPrintf without escape sequence protection
	EndSelect
...
Func __viQueryf($h_session, $s_query, $i_timeout_ms = -1)
	...
	$a_results = DllCall($hVISAdll, "int:cdecl", "viQueryf", "int", $h_session, "str", $s_query, "str", "%t", "str", $s_answer)
	If @error Then Return SetError(@error, @extended, -1)
	Local $errStatus = $a_results[0]
	If $errStatus <> 0 Then
...
Func _viSetAttribute($h_session, $i_attribute, $i_value)
	...
	Local $a_results
	$a_results = DllCall($hVISAdll, "int", "viSetAttribute", "int", $h_session, "int", $i_attribute, "int", $i_value)
	If @error Then Return SetError(@error, @extended, -1)
	Local $errStatus = $a_results[0]
	If $errStatus <> 0 Then
...
Func __viGpibControlREN($h_session, $i_mode)
	...
	Local $a_results
	$a_results = DllCall($hVISAdll, "int", "viGpibControlREN", "int", $h_session, "int", $i_mode)
	If @error Then Return SetError(@error, @extended, -1)
	Local $errStatus = $a_results[0]
	If $errStatus <> 0 Then
