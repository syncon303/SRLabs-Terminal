...
;====================================================================================
; Function Name:   _rxwait2($CommSerialPort, $BufferSize, $firstWaitTime, $charWaitTime, $DEBUG = 0)
; Description:    Recieves data
; Parameters:     $CommSerialPort - value returned by _OpenComm
;				  $BufferSize - Buffer size to wait for
;				  $firstWaitTime - Maximum wait time for first character (in milliseconds)
;				  $charWaitTime - Maximum wait time for following characters (in milliseconds)
;				  $DEBUG - Show debug messages
; Returns:  on success, returns 1
;           on failure returns -1 and sets @error to 1
; Note:
;====================================================================================
Func _rxwait2($CommSerialPort, $BufferSize = 1024, $firstWaitTime = 500,$charWaitTime = 100, $DEBUG = 0)
    if $DEBUG=1 then ConsoleWrite("Wait " & $BufferSize & " " & $firstWaitTime & " " & $charWaitTime & @CRLF)
    Local $rxbuf
    local $jetza=TimerInit(), $charTime, $firstChar = 0
    local $lptr0=dllstructcreate("long_ptr")
	...
	if $rxl>=1 then
	    ...
	    $charTime = TimerInit()
	Else
	    If $firstChar = 0 AND TimerDiff($jetza) > $firstWaitTime Then
		Return($rxbuf)
	    ElseIf $firstChar = 1 AND TimerDiff($charTime) > $charWaitTime Then
		Return($rxbuf)
	    EndIf
	EndIf
    Until stringlen($rxbuf) >= $BufferSize
    Return($rxbuf)
EndFunc
...
; Note:
;====================================================================================
Func _rxwait3($_hCom, $_endStr = "", $_strIn = "", $timeout = 3000, $DEBUG = 0)
    if $DEBUG=1 then ConsoleWrite("_rxWait3: endStr= '" & $_endStr &"', timeout: " & $timeout & @CRLF)
    Local $rxbuf = $_strIn, $size = 4000
    Local $timer=TimerInit()
    Local $lptr0=dllstructcreate("long_ptr")
    Local $rxr, $rxl, $to
    Do
	$rxr=DllCall($commDll,"int","ReadFile","hwnd",$_hCom, _
						"str"," ", _
						"int",1, _
						"long_ptr", DllStructGetPtr($lptr0), _
						"ptr", 0)
	...
	if $rxl>=1 then
	    $rxbuf &= $rxr[2]
	    If $_endStr <> "" AND StringInStr($rxbuf, $_endStr) Then
		if $DEBUG=1 then ConsoleWrite("Head/tail found, exit." &@CRLF)
		Return ($rxbuf)
	    EndIf
	    $size -= $rxl
	ElseIf TimerDiff($timer) > $timeout Then
	    if $DEBUG=1 then ConsoleWrite("Timeout, exit." &@CRLF)
	    Return($rxbuf)
	EndIf
    Until $size <= 0
    Return($rxbuf)
